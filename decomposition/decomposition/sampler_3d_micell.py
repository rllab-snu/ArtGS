...
class Sampler3D:
    ...
    def group_merge(self):
        ...
        if dist_mtx[ind_0][ind_1] < 0.005:
            ...
            #print(deform_weight.shape, label_weight.shape)
            
            #deform_weight = torch.matmul(deform_weight, part_shrink_matrix)
    ...
    def shrink_label(self):
        ...
        unique_label = torch.argwhere(torch.sum(debug_label, 0) > 10)
        unique_label = unique_label.squeeze()
        print(unique_label)
        #unique_label = torch.unique(label_ind)
        """part_shrink_matrix = torch.zeros((self.cfg.model.max_part_num, unique_label.shape[0] + 1), device=self.device)
        print("shrink size: ", part_shrink_matrix.shape)
        for i in range(unique_label.shape[0]):
            part_shrink_matrix[unique_label[i], i] = 1
        for i in range(self.cfg.model.max_part_num):
            if torch.sum(part_shrink_matrix[i, :]) == 0:
                part_shrink_matrix[i, unique_label.shape[0]] = 1"""
        part_shrink_matrix = torch.zeros((label_raw.shape[-1], unique_label.shape[0]), device=self.device)
        print("shrink size: ", part_shrink_matrix.shape)
        for i in range(unique_label.shape[0]):
            part_shrink_matrix[unique_label[i], i] = 1
        #part_shrink_matrix = nn.Parameter(part_shrink_matrix.requires_grad_(False))
        deform_weight = self.model.deform_basis_mat.weight.data.transpose(0, 1)
        deform_weight_shape = deform_weight.shape
        label_weight = self.model.label_basis_mat.weight.data.transpose(0, 1)
        print(deform_weight.shape, label_weight.shape)
        ...
        part_shrink_matrix = torch.zeros((self.model.part_shrink_matrix.shape[1], unique_label.shape[0] + 1), device=self.device)
        for i in range(unique_label.shape[0]):
            part_shrink_matrix[unique_label[i], i] = 1
        for i in range(self.model.part_shrink_matrix.shape[1]):
            if torch.sum(part_shrink_matrix[i, :]) == 0:
                part_shrink_matrix[i, unique_label.shape[0]] = 1
        part_shrink_matrix = torch.matmul(self.model.part_shrink_matrix, part_shrink_matrix).detach()
        self.model.part_shrink_matrix = nn.Parameter(part_shrink_matrix.requires_grad_(False))
        print("shrink size: ", self.model.part_shrink_matrix.shape)"""
    ...
    def get_loss(self, batch_idxs=None):
        ...
        if self.loss_mode == "voxel":
            ...
            #loss = self.l1_loss(emptiness_deformed[valid_idx], emptiness_target.float()[valid_idx])
            if self.step < 5000:
            ...
            #xyzs_source_deform = torch.sum(xyzs_source_deform * valid_labels.transpose(0, 1)[None, :, :, None], 1) # [B, N, 3]
            ...
            valid_idx = density_deformed.bool() | emptiness_target
            loss = self.l2_loss(emptiness_deformed[valid_idx], emptiness_target.float()[valid_idx])
            total_loss = self.batch_size * loss
        else:
            B = self.batch_size
            K = quaternion.shape[1]
            N = xyzs_source.shape[0]
        ...
        if self.adj_loss_version == 1 and self.step > self.n_iters * 0.5:
            nst_idx_flat = self.source_nst_idx[self.source_idx].reshape(-1) # [N, nst_idx] -> [N*nst_idx]
            nst_label_target = torch.argmax(label_raw, 1).reshape(-1, 1).repeat(1, self.num_nst_points).reshape(-1)
            #adj_loss = self.nst_loss_weight * decay_factor * self.cross_loss(label_raw[nst_idx_flat, :], nst_label_target)
            adj_loss = self.nst_loss_weight * self.cross_loss(label_raw[nst_idx_flat, :], nst_label_target)
            total_loss += adj_loss
        elif self.adj_loss_version == 2 and self.step > self.n_iters * 0.3:
            nst_idx_flat = self.source_nst_idx[self.source_idx].reshape(-1) # [N, nst_idx] -> [N*nst_idx]
            nst_label_target = F.softmax(label_raw[nst_idx_flat, :], -1).reshape(N, self.num_nst_points, K).transpose(1, 2) # [N, nst_idx, K]
            #adj_loss = self.nst_loss_weight * decay_factor * self.cross_loss(label_raw[:, :, None].repeat(1, 1, self.num_nst_points), nst_label_target)
            adj_loss = self.nst_loss_weight * decay_factor * self.cross_loss(label_raw[:, :, None].repeat(1, 1, self.num_nst_points), nst_label_target)
            total_loss += adj_loss
        elif self.adj_loss_version == 3:
            HP_target = torch.zeros_like(label_raw)
            nst_idx_flat = self.source_nst_idx[self.source_idx].reshape(-1) # [N, nst_idx] -> [N*nst_idx]
            nst_label_target = label_raw[nst_idx_flat, :].reshape(N, -1, K)
            nst_label_target = torch.mean(nst_label_target, 1).detach()
            adj_loss = self.nst_loss_weight * self.l2_loss(label_raw - nst_label_target, HP_target)
            total_loss += adj_loss
        elif self.adj_loss_version == 4 and self.step > 5000: #and self.step > self.n_iters * 0.5: #and self.step > self.n_iters * 0.3:
            nst_idx_flat = self.source_nst_idx[self.source_idx].reshape(-1) # [N, nst_idx] -> [N*nst_idx]
            ...
            total_loss += adj_loss
        elif self.adj_loss_version == 5 and self.step > 5000:
            nst_idx_flat = self.source_nst_idx[self.source_idx].reshape(-1) # [N, nst_idx] -> [N*nst_idx]
            nst_label_target = F.softmax(label_raw[nst_idx_flat, :], -1).detach().reshape(N, self.num_nst_points, K) # [N, nst_idx, K]
            nst_label_target = torch.mean(nst_label_target, 1) # [N, K]
            loss_nst = self.nst_loss_weight * self.cross_loss(label_raw, nst_label_target)
            total_loss += loss_nst
        elif self.adj_loss_version == 6:
            time_temp_input = frame_time_source[0] * torch.ones((self.gaussian_source.shape[0], 1), device=self.device) # [N, 1]
            label_raw = model.compute_labelfeature(self.gaussian_source, time_temp_input, shrink=False) # [N, K]
            nst_idx_flat = self.gaussian_source_nst_idx.reshape(-1) # [N, nst_idx] -> [N*nst_idx]
            nst_label_target = label_raw[nst_idx_flat, :].reshape(-1, self.num_nst_points, K) # [N, nst_idx, K]
            #HP_target = torch.zeros_like(nst_label_target)
            #adj_loss = self.nst_loss_weight * self.l2_loss(label_raw[:, None, :] - nst_label_target, HP_target)
            nst_label_target = F.softmax(nst_label_target, -1).detach()
            nst_label_target = torch.mean(nst_label_target, 1) # [N, K]
            adj_loss = self.nst_loss_weight * 1.0 * decay_factor * self.cross_loss(label_raw, nst_label_target)
            total_loss += adj_loss
