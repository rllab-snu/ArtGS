    ...
    """def vis_one_source(self, model, iter=0, interactive=False):
        ...
        if self.vis_recon:
            for t in range(0, emptiness_all.shape[0], self.time_step):
                ...
                if self.vis_seg:
                    output_path = f"{folder_name}/segmentation"
                    pcd = o3d.geometry.PointCloud()
                    pcd.points = o3d.utility.Vector3dVector(valid_xyzs)
                    pcd.colors = o3d.utility.Vector3dVector(colors)
                    ply_path = os.path.join(output_path, f"segmentation_{iter}.ply")
                    o3d.io.write_point_cloud(ply_path, pcd)

                output_path = f"{folder_name}/reconstruction"
                pcd = o3d.geometry.PointCloud()
                pcd.points = o3d.utility.Vector3dVector(xyzs_deformed.detach().cpu().numpy())
                pcd.colors = o3d.utility.Vector3dVector(colors)
                ply_path = os.path.join(output_path, f"reconstruction_{iter}_{t}.ply")
                o3d.io.write_point_cloud(ply_path, pcd)"""
                
    def vis_one_source(self, model, xyz_source, iter=0, interactive=False):

        valid_labels = model.compute_labelfeature(xyz_source, torch.zeros_like(xyz_source[:, 0:1]))
        color_list = self.color_list
        valid_xyzs = xyz_source.view(-1, 3).detach().cpu().numpy()
        colors = np.zeros_like(valid_xyzs)
        _, pred_label_ind = torch.max(valid_labels, 1)
        for i in range(valid_xyzs.shape[0]):
            colors[i] = color_list[pred_label_ind[i]] / 255.0

        output_path = f"{self.folder_name}/segmentation"
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(valid_xyzs)
        pcd.colors = o3d.utility.Vector3dVector(colors)
        ply_path = os.path.join(output_path, f"segmentation_{iter}.ply")
