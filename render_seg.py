...
color_palette = torch.FloatTensor(color_palette).to("cuda")
...
def visualize_label_numpy(label, bg_white=True):
    ...
    if bg_white:
        acc = torch.sum(label, dim=-1)
        label = torch.cat((label, 1.0 - acc[..., None]), dim=-1)
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, cam_type):
    ...
    decomposition_path = os.path.join(model_path, name, "ours_{}".format(iteration), "decomposition")
    render_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders")
    gts_path = os.path.join(model_path, name, "ours_{}".format(iteration), "gt")
    makedirs(decomposition_path, exist_ok=True)
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        if idx == 0: time1 = time()
        # breakpoint()
        rendering = render_articulation(view, gaussians, pipeline, background, seg_model, cam_type=cam_type)["render"]
        render_images.append(to8b(rendering).transpose(1, 2, 0))
        render_list.append(rendering)

        rendering = render_label(view, gaussians, pipeline, background, seg_model, cam_type=cam_type)["render"]
        rendering = rendering.permute(1, 2, 0)
        rendering = visualize_label_numpy(rendering)
        #decomposition_images.append(to8b(rendering).transpose(1, 2, 0))
        decomposition_images.append(to8b(rendering))
        decomposition_list.append(rendering.permute(2, 0, 1))
    ...
    multithread_write(decomposition_list, decomposition_path)

    imageio.mimwrite(os.path.join(model_path, name, "ours_{}".format(iteration), 'video_rgb.mp4'), render_images, fps=30)
    imageio.mimwrite(os.path.join(model_path, name, "ours_{}".format(iteration), 'video_decomposition.mp4'), decomposition_images, fps=30)
...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool):
    with torch.no_grad():
        ...
        seg_model = torch.load(f"{scene.model_path}/seg_model.th")
